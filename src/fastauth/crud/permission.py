...
async def create_permission(
    session: AsyncSession, permission_create: PermissionCreate
) -> Permission:
...
async def get_permissions_by_action(
    ...
    result = await session.execute(
        select(Permission).where(Permission.action == action).offset(skip).limit(limit)
    )
    return result.scalars().all()
...
async def permission_exists(
    session: AsyncSession, resource: str, action: str, exclude_id: int | None = None
) -> bool:
